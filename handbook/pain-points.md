...
- **Logic in tests.** (×1) Loops, conditionals, and computation inside test code.
...
- **Solution-space anchoring.** (×6) Fixating on how existing code works
...
- **Production code modified before a test demands it.** (×1) Jumping to
  ...
  around.
- **Dismissing review findings as "pre-existing".** (×2) Labeling issues as
...
- **Dismissing tool feedback without verifying.** (×2) Tool reports a warning,
...
- **Checklists skipped under momentum.** (×20) Established workflows and
